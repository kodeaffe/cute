...
use qt_core::{qs, slot, ContextMenuPolicy, QBox, QObject, QPoint, QStringList, SlotNoArgs};
use qt_widgets::{
...
impl Form {
    fn new() -> Rc<Form> {
        unsafe {
            ...
            let labels = QStringList::from_q_string(&qs("Items"));
            table.set_horizontal_header_labels(&labels);

            let item1 = QTableWidgetItem::new().into_ptr();
            item1.set_text(&qs("Foo"));
            table.set_item(0, 0, item1);

            let item2 = QTableWidgetItem::new().into_ptr();
            item2.set_text(&qs("Bar"));
            table.set_item(1, 0, item2);

            let item3 = QTableWidgetItem::new().into_ptr();
            item3.set_text(&qs("Baz"));
            table.set_item(2, 0, item3);
    ...
    unsafe fn on_button_clicked(self: &Rc<Self>) {
        ...
        QMessageBox::information_q_widget2_q_string(
            &self.widget,
            &qs("Example"),
            &qs("Text: \"%1\".\nCongratulations!").arg_q_string(&text),
        );
    }
